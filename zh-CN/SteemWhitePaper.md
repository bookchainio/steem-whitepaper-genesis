...
即使所有的喂价发布者都是诚实的，仍旧存在这种可能，就是大多数的喂价发布者会被无法控制的事件所影响。 Steem网络被设计为可以容忍短期内喂价中间值的错误，在此期间社区会积极的解决问题。 一个可能需要一些时间来修正的例子就是，短期的市场操纵。 进行长时间的市场操纵是很难并且非常昂贵的。 另外一个例子就是中心化的交易所出错，或者交易所发布的数据损坏。
...
SMD可以在指定的时间内按照公平的价格被转换为STEEM的等值美元，但这并不意味着它可以被看作是可靠的美元替代品。 这些资产需要市场的流动性，在STEEM和SMD之间进行自由转换。 区块链被迫采取措施以防止舞弊造成的可兑换代币的价格低估。 为了弥补这种质量上的损失，区块链可以为货币 流动性提供者 提供固定的成本奖励。 虽然市场操纵和舞弊造成的损失可能是没有上限的，鼓励提现的成本是固定的。
