...
STEEM供应量的增加主要是作为一种会计因素，来避免针对可流动STEEM收取负利率。 负利率会让交易周期复杂化，其原因在于，网络必须针对用户持有STEEM的负利率而调整用户余额。 将区块链的逻辑原样照搬会容易出错，且将集成和应用的过程复杂化。 因此，STEEM的选择就是，从不对用户账户收费，而是增加供应量。 由此就实现了类似的经济效果，而不用强迫每个人在内部账簿中接受STEEM存款来实现负利率。
