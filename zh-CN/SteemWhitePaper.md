...
在第一个月后，Steem矿工会获得Steem Power (SP) 方式的报酬。 SP通过为期两年的"powering down"过程进行结算。 这就意味着，矿工必须要等待一段很长的时间，可能是很多个月，才能将他们挖矿获得的奖励来power down，来收回电费以及计算资源的成本。 这个powering down的过程就会阻止创建矿池，原因在于矿池的操作者不得不在数年内分摊支出。
