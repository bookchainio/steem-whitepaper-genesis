...
如果人们可以在两个方向进行自由转换，交易者可以充分利用区块链的转换率，在不改变价格的情况下进行大额交易。 交易者在看到价格大幅上涨的时候可以以高价（在风险最高的时候）换成SMD，并在价格回调时换回来。 Steem协议通过只允许人们从SMD转换为STEEM，而不是反向转换，来保护社区免受这种舞弊的影响。

区块链决定如何以及何时来创建SMD以及谁会得到它。这会保持创建SMD的稳定速度并消除大多数的舞弊。

### 提现

SMD可以在指定的时间内按照公平的价格被转换为STEEM的等值美元，但这并不意味着它可以被看作是可靠的美元替代品。 这些资产需要市场的流动性，在STEEM和SMD之间进行自由转换。 区块链被迫采取评测措施，以防止舞弊造成的可兑换代币的质量下降。 为了弥补这种质量上的损失，区块链可以为货币提现提供者提供固定的成本奖励。 虽然市场操纵和舞弊造成的损失可能是没有上限的，鼓励提现的成本是固定的。
...
一个用户一旦挂单至少一分钟并被填单，则被看作是提现的供应者。 如果在填单之前用户取消订单，则用户不会被记录为提现供应者。

用户必须在市场双方持续的提供提现以获得奖励。评分算法如下：

      提现点数 = 净报价量 x 净询价量
    

每小时具备最大提现点数的用户会收到1200 STEEM，同时他的提现点数被清零。 在一周内不能赚取任何提现点数账户的提现点数也会被清零。 这意味着无论你提供大额提现或者长期提供小额提现，你都会获得一定比例的奖励金额。 如果净报价量或净询价量两者之一为负，则提现点数被视为0。
