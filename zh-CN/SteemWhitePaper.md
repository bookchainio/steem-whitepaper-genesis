...
- 在第864,000块之后的包含POW的奖励：每块0.0476个STEEM (每轮出块共奖励1个STEEM)，或者每年0.750%，两者取其大。
...
- SMD奖励：SMD会按照一定比例被创建，该比例来自于见证人所设定的APR，SMD会以SMD的形式支付给SMD的持有者。
- 喂价频率的追随：现有SMD总量可兑换的STEEM数额，将根据喂价的变化而变化。 当STEEM的价值 (由喂价来衡量) 增加时，这种变化其实是在销毁 (“燃烧”) STEEM，而当STEEM的价值 (由喂价来衡量) 下降时，这其实是在创造STEEM。
...
这些百分比常数对分配和供给的整体影响是，(大约) 每分钟800个STEEM的速率会保持一定的时间 (例如：STEEM单位)，但按百分比（例如：随着整体供应量越来越大，800个STEEM所占的比例会越来越小）而言是下降的。 当每分钟800个STEEM的速率所涉及的各个组件都达到各自的基于百分比的下限时，相关的组件就会因此而停止下降。 反过来这意味这，从长远来看，STEEM的增长率会从每分钟800个STEEM，增长到 (受时间和网络供应影响) 相应的数值，以便能够维持贡献奖励的恒定10%年增长率，同时也维持贡献奖励和能量奖励的综合的恒定100%年增长率。 其综合效果就是STEEM的年供应量会翻倍 (但是，正如下一节所述，如果大多数用户会Power Up，这种翻倍的效果其实上是一种不转换所有权的“分割”)。
