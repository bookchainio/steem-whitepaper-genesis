...
Sự gia tăng trong cung cấp của STEEM chủ yếu là một tạo phẩm kết toán được tạo ra với mong muốn tránh tính lãi suất âm trên STEEM luân chuyển. Lãi suất tiêu cực sẽ ảnh hưởng một cách phức tạp, và các sàn giao dịch sẽ phải điều chỉnh số dư của người dùng để tính toán tỷ lệ hoàn vốn âm của STEEM được giữ trên sàn. Mirroring the blockchain logic exactly would be error prone and complicate integration and adoption. Therefore, STEEM has chosen to never charge someone's account, but instead to increase supply. This achieves a similar economic result without forcing everyone accepting STEEM deposits to implement negative interest rates on their internal ledger.
